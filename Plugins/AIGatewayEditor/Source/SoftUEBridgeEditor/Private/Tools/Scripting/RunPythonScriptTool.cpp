	...
	const FString& WorldType)
{
	...
	Preamble += TEXT("def _sub_get_editor_property(obj, prop_name, default=None):\n");
	Preamble += TEXT("    if obj is None:\n");
	Preamble += TEXT("        return default\n");
	Preamble += TEXT("    if hasattr(obj, 'get_editor_property'):\n");
	Preamble += TEXT("        try:\n");
	Preamble += TEXT("            return obj.get_editor_property(prop_name)\n");
	Preamble += TEXT("        except Exception:\n");
	Preamble += TEXT("            pass\n");
	Preamble += TEXT("    if hasattr(obj, prop_name):\n");
	Preamble += TEXT("        value = getattr(obj, prop_name)\n");
	Preamble += TEXT("        return value() if callable(value) else value\n");
	Preamble += TEXT("    return default\n");
	Preamble += TEXT("def _sub_set_editor_property(obj, prop_name, value):\n");
	Preamble += TEXT("    if obj is None:\n");
	Preamble += TEXT("        raise AttributeError('Cannot set editor property on None')\n");
	Preamble += TEXT("    if hasattr(obj, 'set_editor_property'):\n");
	Preamble += TEXT("        obj.set_editor_property(prop_name, value)\n");
	Preamble += TEXT("        return None\n");
	Preamble += TEXT("    setattr(obj, prop_name, value)\n");
	Preamble += TEXT("    return None\n");
	Preamble += TEXT("def _sub_get_generator_class(obj):\n");
	Preamble += TEXT("    if obj is None:\n");
	Preamble += TEXT("        return None\n");
	Preamble += TEXT("    if isinstance(obj, str) and hasattr(_sub_unreal, 'EditorAssetLibrary'):\n");
	Preamble += TEXT("        try:\n");
	Preamble += TEXT("            return _sub_unreal.EditorAssetLibrary.load_blueprint_class(obj)\n");
	Preamble += TEXT("        except Exception:\n");
	Preamble += TEXT("            return None\n");
	Preamble += TEXT("    for _prop in ('generated_class', 'GeneratedClass'):\n");
	Preamble += TEXT("        _value = _sub_get_editor_property(obj, _prop, None)\n");
	Preamble += TEXT("        if _value:\n");
	Preamble += TEXT("            return _value\n");
	Preamble += TEXT("    if hasattr(obj, 'get_path_name') and hasattr(_sub_unreal, 'EditorAssetLibrary'):\n");
	Preamble += TEXT("        try:\n");
	Preamble += TEXT("            _asset_path = obj.get_path_name().split('.')[0]\n");
	Preamble += TEXT("            return _sub_unreal.EditorAssetLibrary.load_blueprint_class(_asset_path)\n");
	Preamble += TEXT("        except Exception:\n");
	Preamble += TEXT("            return None\n");
	Preamble += TEXT("    return None\n");
	Preamble += TEXT("if not hasattr(_sub_unreal, 'get_editor_property'):\n");
	Preamble += TEXT("    _sub_unreal.get_editor_property = _sub_get_editor_property\n");
	Preamble += TEXT("if not hasattr(_sub_unreal, 'set_editor_property'):\n");
	Preamble += TEXT("    _sub_unreal.set_editor_property = _sub_set_editor_property\n");
	Preamble += TEXT("if not hasattr(_sub_unreal, 'get_generator_class'):\n");
	Preamble += TEXT("    _sub_unreal.get_generator_class = _sub_get_generator_class\n");
