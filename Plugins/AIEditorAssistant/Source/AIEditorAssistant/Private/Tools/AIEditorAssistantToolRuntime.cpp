...
namespace
{
    ...
    FAIEditorAssistantToolResult ConvertBridgeResult(const FString& ToolName, const FBridgeToolResult& BridgeResult)
    {
        FString MessageText;
        ...
        return BridgeResult.bIsError
            ? FAIEditorAssistantToolResult::Error(MakeSummary(ToolName, MessageText), bHasPayload ? ParsedPayload : nullptr)
            : FAIEditorAssistantToolResult::Success(MakeSummary(ToolName, MessageText), bHasPayload ? ParsedPayload : nullptr);
    }
    ...
    EAIEditorAssistantToolConfirmationPolicy GetConfirmationPolicy(const FString& ToolName)
    {
        return ToolName == TEXT("delete-actor") ||
            ...
            ? EAIEditorAssistantToolConfirmationPolicy::ExplicitApproval
            : EAIEditorAssistantToolConfirmationPolicy::None;
    }
...
FAIEditorAssistantToolResult FAIEditorAssistantToolResult::Success(const FString& InSummary, const TSharedPtr<FJsonObject>& InPayload)
{
    FAIEditorAssistantToolResult Result;
...
FAIEditorAssistantToolResult FAIEditorAssistantToolResult::Error(const FString& InSummary, const TSharedPtr<FJsonObject>& InPayload)
{
    FAIEditorAssistantToolResult Result;
    Result.Summary = InSummary;
    Result.Payload = InPayload;
...
FAIEditorAssistantToolResult FAIEditorAssistantToolResult::Rejected(const FString& InSummary)
{
    FAIEditorAssistantToolResult Result = Error(InSummary);
    Result.bWasRejected = true;
    return Result;
}

FString FAIEditorAssistantToolResult::ToMessageContent() const
{
...
FAIEditorAssistantToolRuntime& FAIEditorAssistantToolRuntime::Get()
{
    static FAIEditorAssistantToolRuntime Instance;
    return Instance;
}

void FAIEditorAssistantToolRuntime::Startup()
{
    if (bStarted)
    {
        return;
...
void FAIEditorAssistantToolRuntime::Shutdown()
{
    bStarted = false;
}
...
FAIEditorAssistantToolResult FAIEditorAssistantToolRuntime::ExecuteTool(const FString& ToolName, const TSharedPtr<FJsonObject>& Arguments) const
{
    if (ToolName == TEXT("get-selected-actors"))
    {
        ...
        if (!Selection)
        {
            return FAIEditorAssistantToolResult::Error(TEXT("Actor selection is not available."));
        }
        ...
        return FAIEditorAssistantToolResult::Success(FString::Printf(TEXT("Found %d selected actor(s)."), Actors.Num()), Payload);
    }

    FBridgeToolContext Context;
    Context.RequestId = TEXT("ai-editor-assistant-chat");
    ...
    if (ToolName == TEXT("set-blueprint-default"))
    {
        ...
        if (UBlueprint* Blueprint = LoadObject<UBlueprint>(nullptr, *AssetPath))
        {
            ...
            if (BlueprintCDO != nullptr)
            {
                ...
                {
                    return FAIEditorAssistantToolResult::Error(FString::Printf(
    ...
    if (ToolName == TEXT("set-actor-transform"))
    {
        ...
        if (!Actor)
        {
            return FAIEditorAssistantToolResult::Error(FString::Printf(TEXT("Actor '%s' was not found."), *ActorName));
        }
        ...
        return FAIEditorAssistantToolResult::Success(FString::Printf(TEXT("Updated transform for '%s'."), *Actor->GetName()), Payload);
    }
    ...
    return FAIEditorAssistantToolResult::Error(FString::Printf(TEXT("Unknown tool '%s'."), *ToolName));
}

void FAIEditorAssistantToolRuntime::BuildDefinitions()
{
    Definitions.Reset();
    DefinitionIndexByName.Reset();

    auto AddDefinition = [this](const FString& Name, const FString& Description, const TSharedPtr<FJsonObject>& Parameters, const EAIEditorAssistantToolConfirmationPolicy ConfirmationPolicy = EAIEditorAssistantToolConfirmationPolicy::None)
    {
        ...
        FAIEditorAssistantToolDefinition Definition;
